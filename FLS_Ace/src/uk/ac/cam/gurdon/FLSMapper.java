...
public class FLSMapper {
	...
	private static ImagePlus segment(ImagePlus imp, double sigma, double k, String threshold) {
		try {
			...
			GaussianBlur3D.blur(mask, sigma, sigma, 0d); // Use 3D Gaussian to apply a 2D Gaussian blur. Threading is
															// broken in 2D Gaussian class.
			...
			// IJ.run(mask, "Convert to Mask", "method="+threshold+" background=Dark
			// black"); //throws Exception about 8-bit stack
	...
	public static ArrayList<FLS>[] map(ImagePlus imp, double sigma, double k, String threshold, int base,
			double minLength, double maxDist) {
		int W = imp.getWidth();
		int H = imp.getHeight();
		int Z = imp.getNSlices();
		int T = imp.getNFrames();
		...
		try {
			...
			/*
			 * ////circularity histogram values int bins = 20; int[] circH = new int[bins];
			 * int max = 0; for(int i=0;i<bins;i++){ circH[i] = 0; } for(int t=0;t<T;t++){
			 * for(int f=0;f<flsArr[t].size();f++){ double circ =
			 * flsArr[t].get(f).circularity(); int ind = (int)Math.floor((circ*bins));
			 * circH[ind-1]++; max = (int)Math.max(max,circH[ind-1]); } }
			 * 
			 * ////circularity histogram
			 * //IJ.log(Arrays.toString(circH).replaceAll("[\\[\\]]","")); int hw = 500; int
			 * hh = 500; int bar = hw/bins; int border = 50; int[] normH = new int[bins];
			 * for(int i=0;i<bins;i++){ double d = (((double)circH[i])/max)*hh; normH[i] =
			 * (int)Math.round(d); } Image hist = new
			 * BufferedImage(hw+(border*2),hh+(border*2),BufferedImage.TYPE_INT_RGB);
			 * Graphics2D g2d = (Graphics2D)hist.getGraphics(); g2d.setColor(Color.BLACK);
			 * g2d.fillRect(0,0,hw+(border*2),hh+(border*2)); for(int i=0;i<bins;i++){
			 ...
			 * g2d.drawString("n",10,border+(hh/2)); for(double y=0d;y<=1d;y+=0.1d){
			 * g2d.drawString(IJ.d2s((1d-y)*max,0),20,border+Math.round(y*hh)+5); //y labels
			 * } ImagePlus histogram = new
			 * ImagePlus("Circularity Histogram : "+imp.getTitle(),hist); histogram.show();
			 * ////
			 */
